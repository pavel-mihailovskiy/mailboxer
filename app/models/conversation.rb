class Conversation < ActiveRecord::Base
  ...
  scope :archived_for_user, lambda { |user| includes(:archived_conversation).where('archived_conversations.user_id = ?', user.id) }
  scope :chat_conversations_between, lambda { |friend, user|
    joins("LEFT OUTER JOIN notifications ON notifications.conversation_id = conversations.id AND notifications.type IN ('Message')").
    where("(select count(*) from receipts where receipts.notification_id = notifications.id
    AND (receipts.receiver_id = #{friend.id} or receipts.receiver_id = #{user.id})) = 2 AND chat = true")
  }  
