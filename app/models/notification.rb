class Notification < ActiveRecord::Base
  ...
  validate :presence_of_body_or_attachment
  validate :check_recipients, :if => Proc.new { recipients.any? }
  ...
  def check_recipients
    blockers = recipients & User.blockers_of(sender)
    errors.add(:recipients, I18n.t('activerecord.errors.models.conversation.attributes.can_not_send_message_to_blokers', :users => blockers.map(&:fullname).join(','))) if blockers.any?
  end

  def presence_of_body_or_attachment
    errors.add(:body_or_attachment, I18n.t('activerecord.errors.models.conversation.attributes.no_body_or_attachment')) unless self.body.present? || self.attachment.present?
  end
end
